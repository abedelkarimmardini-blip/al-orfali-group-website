  ...
  return (
    <section ref={heroRef} className="relative min-h-[100svh] flex items-end overflow-hidden">
      <div ref={imageRef} className="absolute inset-0 scale-[1.06] transition-transform duration-700 ease-out">
        <Image src="/images/official/hero-official.jpg" alt="Al Orfali H7 Tower" fill priority sizes="100vw" className="object-cover object-center" />
      </div>
      <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(5,5,4,.94)_0%,rgba(5,5,4,.54)_48%,rgba(5,5,4,.14)_100%)]" />
      <div className="absolute inset-0 bg-gradient-to-t from-[#101D2B] via-transparent to-black/35" />
      <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-[#E4EBEF]/40 to-transparent" />

      <div className="relative z-10 w-full max-w-[1440px] mx-auto px-5 sm:px-8 lg:px-12 pb-10 md:pb-16 pt-32">
        <div className="max-w-4xl">
          <div className="inline-flex items-center gap-3 text-[10px] tracking-[0.28em] uppercase text-[#E4EBEF] mb-7">
            <span className="w-1.5 h-1.5 rounded-full bg-[#E4EBEF] animate-pulse" />
            H7 Tower · Marota City
          </div>
          <h1 className="font-serif text-[3.4rem] sm:text-7xl lg:text-[6.8rem] font-normal text-white leading-[0.88] tracking-[-0.045em] mb-7">
            Architecture<br />
            <span className="text-[#E4EBEF] italic">made timeless.</span>
          </h1>
          <p className="text-white/60 text-sm md:text-base leading-relaxed mb-9 max-w-lg">
            A sculptural residential landmark where fluid architecture, expansive glass, and considered landscapes shape an exceptional way of living.
          </p>
          <div className="flex items-center gap-3 flex-wrap mb-12">
            <button onClick={() => scrollTo('#units')} className="magnetic bg-[#F2F4F2] hover:bg-[#FFFFFF] text-[#11100d] text-[10px] font-semibold uppercase tracking-[0.18em] px-7 py-4 transition-colors">
              Discover residences
            </button>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-y-5 border-t border-white/15 pt-6 max-w-2xl">
            {STATS.map((stat, index) => (
              <div key={stat.label} className={index > 0 ? 'sm:pl-5 sm:border-l border-white/10' : ''}>
                <div className="font-serif text-2xl text-[#E4EBEF]">{stat.num}</div>
      ...
      <div className="absolute bottom-8 right-8 z-10 hidden md:flex items-center gap-3 text-[9px] tracking-[0.2em] uppercase text-white/35">
        Scroll to discover <span className="block w-14 h-px bg-[#E4EBEF]/50" />
      </div>
    </section>
  )
}
